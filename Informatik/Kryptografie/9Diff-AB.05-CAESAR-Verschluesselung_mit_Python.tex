...
\begin{rahmen}\textbf{Zur Erinnerung einige Python-Befehle}\small
	\begin{tasks}[style=itemize](2)
		\task\code{print(\textquotedbl Hallo, Welt!")} Gibt den Text \enquote{Hallo, Welt!} aus.
		\task\code{input(\textquotedbl Gib etwas ein ein: ")} Gibt den Text \enquote{Gib etwas ein:} aus
			  und wartet, bis der Nutzer die ENTER-Taste betätigt.
		\task\code{ord(\textquotedbl A")} Wandelt den Buchstaben \enquote{A} in seinen Unicode (hier \enquote{65}) um.
		\task\code{chr(65)} Wandelt den Unicode \enquote{65} in das passende Zeichen (hier \enquote{A}) um.
		\task\code{\textquotedbl Ein Text".upper()} Wandelt \enquote{Ein Text} in \enquote{EIN TEXT} um.
		\task\code{\textquotedbl Ein Text".lower()} Wandelt \enquote{Ein Text} in \enquote{ein text} um.

		\task*\code{for x in \textquotedbl Hallo, Welt":} Setzt \code{x} auf jedes Zeichen in \enquote{Hallo, Welt}
