...
\begin{smallitemize}
	\item \code{public void drehe( String richtung )}
	\item \code{public boolean huegelVorhanden( String richtung )}
\end{smallitemize}
...
Implementiert eine Methode \code{public void fahreStrecke( int strecke )}, die den Rover bei jedem Aufruf so viele Felder fahren lässt, wie in der \emph{Parametervariablen} \code{strecke} übergeben wird.

\subsubsection*{Aufgabe 3}
Implementiert eine Methode \code{public void fahreRechteck( int kante )}, die die Methode \code{fahreStrecke} von oben benutzt, um den Rover ein Rechteck mit der Kantenlänge \code{kante}) fahren zu lassen.
...
\begin{smallitemize}
	\item \code{public boolean gesteinVorhanden()}
	\item \code{public boolean markeVorhanden()}
\end{smallitemize}
...
Methoden mit einer \emph{Rückgabevariablen} müssen eine \code{return}-Anweisung enthalten, die den Wert kennzeichnet, der \emph{zurück gegeben} werden soll. z.B. \code{return 5;}

\subsubsection*{Aufgabe 1}
Überlegt gemeinsam, welchen Vorteil \emph{Rückgabevariablen} haben.

\subsubsection*{Aufgabe 2}
Der Rover steht auf einem Feld mit einer unbekannten Anzahl Marken. Implementiert eine Methode \code{public int zaehleMarken()}, die den Rover die Marken auf dem Feld zählen lässt, und die Anzahl als \emph{Rückgabe} hat. Der Rover muss sich dafür nicht bewegen.

\hinweis{Ihr könnt in Greenfoot mehrere Marken auf ein Feld legen.}

\subsubsection*{Aufgabe 3}
Implementiert eine Methode \code{public int zaehleGesteine()}, die den Rover fünf Felder fahren, und auf dem Weg
