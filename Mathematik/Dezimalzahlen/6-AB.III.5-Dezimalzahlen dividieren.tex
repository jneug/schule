...
\begin{document}
	...
	\begin{aufgabe}[symbol=\symPartner] 
		\label{aufg:1}Finde die Rechnungen mit demselben Ergebnis und markiere sie.
		...
		\begin{tikzpicture}[xscale=2]
			...
			\node at (2.5,4.2){$2:0,5$};
			\node at (4.2,4.7){$200:500$};
			\node at (5.1,3.9){$0,2:0,5$};
		\end{tikzpicture}
		\end{center}
		
		\tipp{Forme die Rechnungen in Brüche um.}
	\end{aufgabe}

	\begin{aufgabe}[symbol=\symPartner]
		Betrachte die Rechnungen und Ergebnisse aus \prettyref{aufg:1}. Kannst du Regelmäßigkeiten erkennen?\smallskip
		
		Formuliere eine Regel, wann das Ergebnis verschiedener \emph{Divisionen von Dezimalzahlen}, gleich ist.\smallskip
		
		\feldLin[1cm]{4}
		
		\hinweis{Achte auf die Position des Kommas.}
	\end{aufgabe}

	\begin{aufgabe}[symbol=\symPartner]
		Notiere vier Divisionen, die dasselbe Ergebnis haben.
